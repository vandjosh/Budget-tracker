...
function rolloverIfNeeded() {
  ...
  if (lastActiveMonth === currentMonth) return;
...
function updateSummary() {
  ...
  if (budgetTotal) budgetTotal.textContent = formatMoney(baseBudget);
  if (rolloverTotal) rolloverTotal.textContent = formatMoney(rollover);
  if (spentTotal) spentTotal.textContent = formatMoney(spent);
  if (committedTotal) committedTotal.textContent = formatMoney(commitments);
  if (remainingTotal) remainingTotal.textContent = formatMoney(remaining);
  if (availableTotal) availableTotal.textContent = `$${formatMoney(available)}`;

  if (transactionCount) transactionCount.textContent = getExpensesForMonth(currentMonthKey).length;
  if (recurringCount) recurringCount.textContent = recurringBills.length;
  if (largestExpense) largestExpense.textContent = `$${formatMoney(getLargestExpenseThisMonth())}`;
  ...
  if (statusBadge) {
    ...
    if (remaining > available * 0.35) {
      if (progressFill) progressFill.classList.add("progress-good");
      statusBadge.classList.add("status-good");
      statusBadge.textContent = "Healthy";
    } else if (remaining > 0) {
      if (progressFill) progressFill.classList.add("progress-warning");
      statusBadge.classList.add("status-warning");
      statusBadge.textContent = "Tight";
    } else {
      if (progressFill) progressFill.classList.add("progress-danger");
...
function resetRecurringForm() {
  ...
  const submitButton = recurringForm.querySelector("button[type='submit']");
  if (submitButton) submitButton.textContent = "Add Recurring Bill";
}
...
function renderRecurringBills() {
  ...
  sortedBills.forEach((bill) => {
    ...
    deleteBtn.addEventListener("click", () => {
      ...
      expenses = expenses.filter((expense) => !(expense.isRecurring && expense.sourceRecurringId === bill.id));

      if (wasEditingThisBill) resetRecurringForm();
